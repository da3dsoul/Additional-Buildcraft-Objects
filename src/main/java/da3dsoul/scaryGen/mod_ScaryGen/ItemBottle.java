		...
		if (hasCaptured(itemstack) && itemstack.stackSize == 1)
		{
			...
			if(ent instanceof EntityMinecart)
			{
				...
				if(l != 0)
				{
					...
					i1.addCollisionBoxesToList(world, i, j, k, AxisAlignedBB.getBoundingBox(i - 0.5, j + 0.475, k - 0.5, i + 0.5, j + 1.25, k + 0.5), list, null);
			...
			ent.setLocationAndAngles(i + 0.5, j + d, k + 0.5, MathHelper.wrapAngleTo180_float(world.rand.nextFloat() * 360F), 0.0F);
