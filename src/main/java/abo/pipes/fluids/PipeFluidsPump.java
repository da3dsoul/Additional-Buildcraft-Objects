...
public class PipeFluidsPump extends ABOPipe<PipeTransportFluids> {
	...
	public PipeFluidsPump(Item itemID) {
		this(itemID, Blocks.water);
	}

	public PipeFluidsPump(Item itemID, Block liquidStill) {
	...
	public void updateEntity() {
		super.updateEntity();
		if (container.getWorldObj().getBlock(container.xCoord, container.yCoord - 1, container.zCoord) == liquid) {
			transport.fill(ForgeDirection.DOWN, new FluidStack(BlockUtil.getFluid(liquid), FluidContainerRegistry.BUCKET_VOLUME / 10), true);
		}
	}
